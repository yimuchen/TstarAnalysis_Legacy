#ifndef dMRecoTstar_H
#define dMRecoTstar_H
...
double dMRecoTstar(
        ...
        int NPhotons, int P_Index[], int Nu_mode){ // neutrino's solution mode [0,1,2] = [chi2, smaller,larger]
                    ...
                    for(int j3=0;j3<NJets;j3++){
                        ...
                        for(int j4=0;j4<NJets;j4++){
                            ...
                            double chi2_2 =
                                ...
                                pow(topquark[0].Mag()-top_MASS,2)/pow(massRes[1],2)+
                                pow(topquark[1].Mag()-top_MASS,2)/pow(massRes[2],2)+
                                pow(tstar2[0].Mag()-tstar2[1].Mag(),2)/pow(massRes[3],2);

                            if (chi2_1 < chi2_){
                                chi2_ = chi2_1;
                                dMtstar_ = fabs(tstar1[1].Mag() - tstar1[0].Mag())/(tstar1[1].Mag() + tstar1[0].Mag());
                            }
                            if (chi2_2 < chi2_){
                                chi2_ = chi2_2;
                                dMtstar_ = fabs(tstar2[1].Mag() - tstar2[0].Mag())/(tstar2[1].Mag() + tstar2[0].Mag());
                            }
