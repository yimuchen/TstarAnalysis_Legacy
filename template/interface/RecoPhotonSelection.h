    ...
    for(int nl=0;nl<PhotonInfo.Size;nl++){
        ...
        if(     
                ...
                ((// Barrel
                  ...
                  (max(PhotonInfo.phoPFChIsoDR03[nl]-EffectiveAreaPhoton(PhotonInfo.Eta[nl],0)*RhoPU,(float)0.) < UsingPhoISO*PFCHISOEB_) &&
                  (max(PhotonInfo.phoPFNeuIsoDR03[nl]-EffectiveAreaPhoton(PhotonInfo.Eta[nl],1)*RhoPU,(float)0.)< UsingPhoISO*(PFNHISOEB_+0.04*PhotonInfo.Pt[nl]))
		  &&
                  (max(PhotonInfo.phoPFPhoIsoDR03[nl]-EffectiveAreaPhoton(PhotonInfo.Eta[nl],2)*RhoPU,(float)0.)< UsingPhoISO*(PFPhoISOEB_+0.005*PhotonInfo.Pt[nl]))
                  )
                 ||
                 (// Endcap
                  ...
                  PhotonInfo.passelectronveto[nl] &&
                  PhotonInfo.hadTowOverEm[nl] <=0.05 &&
                  (max(PhotonInfo.phoPFChIsoDR03[nl]-EffectiveAreaPhoton(PhotonInfo.Eta[nl],0)*RhoPU,(float)0.) < UsingPhoISO*PFCHISOEE_) &&
                  (max(PhotonInfo.phoPFNeuIsoDR03[nl]-EffectiveAreaPhoton(PhotonInfo.Eta[nl],1)*RhoPU,(float)0.)< UsingPhoISO*(PFNHISOEE_+0.04*PhotonInfo.Pt[nl])) &&
                  (max(PhotonInfo.phoPFPhoIsoDR03[nl]-EffectiveAreaPhoton(PhotonInfo.Eta[nl],2)*RhoPU,(float)0.)< UsingPhoISO*(PFPhoISOEE_+0.005*PhotonInfo.Pt[nl]))
                 ))
          ){
            // Photon cleaning
            int dup_flag = 0;
            // comment on 02/18/2014 from Ming
            ///*
