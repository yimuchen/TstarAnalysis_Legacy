...
namespace {
    ...
    int jetsFromTTbar(const GenInfoBranches &gen, const JetInfoBranches &jet, const std::vector<int> jetIndeces) {
      //0 for not from ttbar, +/-1 for from ttbar, +/-2 for t'tbar'
      if(jetIndeces.size()==0) {
	printf("empty jet list\n");
	return 0;
      }
      int nFromT = 0, nFromTbar = 0;
      int nFromTp = 0, nFromTbarp = 0;
      for(unsigned i=0; i<jetIndeces.size(); i++) {
	int origID = originatorPdgID<JetInfoBranches>(gen,jet,jetIndeces[i]);
	if(origID ==  6) nFromT++;
